...
public class Knapsack2 {
    ...
    private final KnapsackObject[] aKnap;
    ...
    * @param alpha the probability of an object being mutated
     ...
     **/
    public Knapsack2(int seed, int lambda, int k, double alpha){
...
//            }
            ...
            pop = elimination(pop);
//            Collections.sort(pop, compValue()); // Do we need this?
...
//        }
        ...
        for (i = 0; i < lambda % k; i++) {
            selected.add(pop.get(lambda - i - 1));
        }
     ...
     *
     * @param src selected individuals, with size mu
     * @param dest offsprings, with size of interger closet to mu/2
    ...
    public void recomMutate(ArrayList<ArrayList<Integer>> src, ArrayList<ArrayList<Integer>> dest) {
        ...
        while (count <= src.size() - 1) {
            temp = new ArrayList<Integer>();
            temp = recombination(src.get(count), src.get(count + 1));
            mutate(temp, alpha);
            dest.add(kOptimizer(temp));
            count += 2;
        }

        // If mu is an odd number, add the last element of src to dest
        if (count > src.size()) {
            dest.add(kOptimizer(src.get(src.size() - 1))); // Get the last element, add it to dest
        }
    }
    ...
    /**
     * using k-opt strategy for local search operator
     * @param candidate
     * @return the best candidate solution within the neighborhood
     */

    private ArrayList<Integer> kOptimizer(ArrayList<Integer> candidate) {
        ...
        for (int j = 0; j < neighbors.size(); j++) {
            val = fitness(neighbors.get(j));
            cost = cost(neighbors.get(j));
            if (val > max && cost <= capacity) {
                max = val;
                champion = j;
            }
        }

        if (champion == -1) return candidate;
        return neighbors.get(champion);
    }
    ...
    public static void main(String[] args) {
        ...
        Knapsack2 test = new Knapsack2(nObject, lambda, k, alpha);
        Knapsack testOld = new Knapsack(nObject, lambda, k, alpha);
        testOld.aKnap = test.aKnap;

        System.out.println("Capacity is " + test.capacity);
//        test.getPopulation();
//        System.out.println(test.pop);
//        selected = test.selection(3);
//        System.out.println(selected);
//        for (ArrayList<Integer> a : selected) {
//            System.out.println(selected);
//        }
        test.generateSolution();
        testOld.generateSolution();
        System.out.println("Final solution of NEW TEST contains the following objects " + test.pop.get(lambda - 1));
        System.out.println("Total value is: " + test.fitness(test.pop.get(0)) + " Capacity is :" + test.cost(test.pop.get(0)));
        System.out.println("Final solution of OLD TEST contains the following objects " + testOld.pop.get(lambda - 1));
        System.out.println("Total value is: " + testOld.fitness(testOld.pop.get(lambda - 1)) + " Capacity is :" + testOld.cost(testOld.pop.get(lambda-1)));

        // Conclusion: this local optimizer gets stuck very easily at a local minima => more randomized aspect should be added.
    }
}
